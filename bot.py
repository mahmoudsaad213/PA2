...
def key(message):
    ...
    try:
        ...
        bot.reply_to(message, "📝 Please send: <code>/key YOUR_KEY</code>", parse_mode='HTML')
...
def handle_file(message):
    ...
    if not is_authorized(user_id):
        bot.reply_to(message, "🔒 Please authorize first: <code>/key YOUR_KEY</code>", parse_mode='HTML')
        return
    try:
...
def process_cards(user_id, chat_id):
    ...
    for i in range(0, len(cards), 3):
        ...
        for j, card in enumerate(batch):
            ...
            eta = ((len(cards) - (i + j + 1)) * 2.5)  # افتراض 2.5 ثانية لكل بطاقة
...
def update_progress(user_id, chat_id, live_count, otp_count, declined_count, error_count, progress, eta=0, speed=0, current_card=""):
    markup = InlineKeyboardMarkup()
    ...
    try:
        bot.edit_message_text(msg, chat_id, progress_messages[user_id], parse_mode='HTML', reply_markup=markup)
    except:
        pass
